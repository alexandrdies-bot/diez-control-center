...
Добавлен менеджерский workflow заказа без изменения DB schema: `PATCH /orders/:id/status` меняет статус заказа в существующем наборе `new`, `confirmed`, `in_work`, `ready`, `completed`, `canceled`, а `POST /orders/:id/comments` пишет комментарий менеджера в `order_events` как `comment_added`. Desktop показывает блок `Работа менеджера` в деталях заказа и обновляет ленту/детали после сохранения.
