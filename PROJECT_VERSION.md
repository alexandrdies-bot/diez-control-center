...
CDEK selected delivery save добавлен как backend endpoint `PATCH /orders/:id/delivery/cdek` для manager/admin. Endpoint сохраняет delivery summary в существующие `app.order_delivery` и `app.orders.delivery_total_minor`, пересчитывает `total_price_minor`, пишет существующий `order_updated` event type и сохраняет Ozon financial protections: active Ozon payments отменяются через существующий cancellation flow при финансовом изменении, но новая Ozon Pay ссылка автоматически не создаётся; менеджер создаёт её вручную после проверки доставки. Final/financial Ozon payments блокируют изменение. Response включает `payment.financialChanged`, `payment.activePaymentsCanceled`, `payment.canceledPaymentIds` и `payment.deliveryNeedsReview`. CDEK API, OAuth, shipment creation, shipment tables и migrations не используются.

Desktop delivery panel теперь может сохранить выбранную СДЭК-доставку через отдельную кнопку `Сохранить доставку`. UI собирает payload из выбранного города, ПВЗ, тарифа, габаритов и calculation response, показывает сохранённую стоимость доставки и новый итог заказа, предупреждает об отменённой active Ozon payment и коротко показывает final payment block / Ozon cancel failed errors. Desktop не ходит напрямую в CDEK, не создаёт shipment, не создаёт новую Ozon payment и не меняет order status автоматически.

CDEK save delivery зафиксирован как предварительный этап расчёта суммы заказа: `Маршрут -> ПВЗ -> груз -> тариф -> сохранить доставку в заказ -> пересчитать итог -> подготовить оплату`. Доставка хранится отдельно и отображается как визуальная строка под товарными позициями плюс строка в итогах, не как товарная позиция `order_items`; итог остаётся моделью `товары + доставка`. Если товарные позиции меняются при уже сохранённой доставке, delivery помечается как `needsReview`, а новая Ozon Pay ссылка блокируется до повторной проверки и сохранения доставки. После нового сохранения доставки менеджер создаёт ссылку вручную; backend не делает auto-recreate оплаты. Сохранение доставки не создаёт реальное отправление СДЭК, трек-номер, накладную, документы или этикетки. Future shipment flow с отдельным действием `Создать отправление в СДЭК` должен появиться позже и будет отвечать за заявку, трек, документы и этикетки; будущая кнопка должна быть доступна только после оплаты заказа, при сохранённой СДЭК-доставке без `needsReview` и до создания реального отправления.
...
Desktop CDEK delivery UI теперь использует summary-first режим для уже сохранённой доставки: менеджер сначала видит компактную карточку `СДЭК-доставка сохранена`, отметку `Отправление СДЭК ещё не создано`, маршрут, тариф, стоимость, срок и груз. Полная форма выбора городов/ПВЗ/тарифов открывается только по кнопкам изменения/пересчёта, статус интеграции стал маленькой технической строкой, а списки результатов ограничены по высоте и сворачиваются после выбора. Backend API contracts, DB, Ozon-flow, shipment lifecycle и provider calls не менялись.

CDEK edit/new flow в Desktop теперь оформлен как обычная форма доставки, а не как пошаговый технический конструктор. Основные блоки: `Маршрут`, `Груз`, `Тарифы`, `Расчёт и сохранение`. В маршруте город отправителя и город получателя видны сразу как поля ввода, autocomplete запускается с debounce и показывает dropdown под активным полем, а выбор ПВЗ/пункта открывается только для выбранного города как select-like field. Нижняя общая кнопка `Сохранить` не считается действием для CDEK.

CDEK edit mode для сохранённой доставки теперь открывается в заполненном compact-state: `Изменить доставку` восстанавливает sender/recipient points, тариф, стоимость, срок и груз из saved provider payload, не раскрывает lists автоматически и использует единый active picker для городов, пунктов и тарифов. CDEK-блок теперь работает как форма оформления доставки: города вводятся прямо в блоке маршрута и ищутся autocomplete dropdown с debounce, ПВЗ/пункты показываются только в активном select-like выборе, тарифы выводятся карточками после заполнения маршрута и груза, а сохранённый расчёт остаётся помеченным как сохранённый, пока менеджер реально не меняет маршрут, груз или тариф.

CDEK delivery UI dropdowns теперь работают как overlay, а не как встроенные списки внутри формы. City, sender point, recipient PVZ и tariff dropdowns не двигают блоки `Груз`, `Тарифы`, `Расчёт и сохранение`; ПВЗ/пункты имеют локальный поиск по адресу и счётчик показанных пунктов; тарифы отображаются вертикальным списком с ценой справа и secondary `#code`. Desktop не режет видимые ПВЗ/тарифы первыми несколькими элементами, а backend/API contracts, provider calls, DB, Ozon и shipment lifecycle не менялись.

CDEK tariff selection в Desktop теперь оформлен как отдельный блок по логике личного кабинета СДЭК, а не как dropdown. В блоке показаны режимы `Откуда` и `Куда доставить`: активен текущий поддержанный сценарий `из ПВЗ -> в ПВЗ`, остальные варианты отображаются disabled. После подбора тарифы остаются в постоянном вертикальном списке с названием, сроком, ценой справа и secondary tariff code; выбранная строка подсвечивается и используется последующим расчётом.

CDEK tariff list загружается автоматически после заполнения sender city/point, recipient city/PVZ и габаритов груза. Desktop строит fingerprint маршрута и груза, чтобы не повторять запрос без изменения данных; при изменении маршрута/груза старый список и выбранный тариф сбрасываются. Основной список фильтруется под текущий сценарий `из ПВЗ -> в ПВЗ` (`склад-склад` / delivery mode 4), остальные provider варианты остаются в свернутом `Другие варианты`. Если tariff list содержит цену и срок, выбор строки сразу формирует calculation state для сохранения доставки без обязательного отдельного расчёта.

Для диагностики missing `Посылка склад-склад` backend `POST /cdek/tariffs` пишет безопасный structured log только по нормализованным тарифам: total/valid/errored counts, tariff code/name/mode/error flags, presence `#136`/`Посылка` и склад-склад counts. Лог не содержит Authorization, bearer/OAuth token, CDEK credentials, raw env, raw provider request/response или персональные данные.
...
Личный кабинет заказчика зафиксирован как временное рабочее пространство для выполнения, обсуждения и доработки заказов, а не вечный файловый архив. Ориентир хранения — 3 месяца после последней активности; автоочистка пока не включена. Retention-поля подготовлены, `retention_locked` означает запрет будущей автоочистки для постоянных клиентов, рекламных агентств и важных заказчиков.

Тяжёлые файлы заказов не должны храниться вечно, но пока заказ активен, файлы нельзя автоматически удалять. Автоочистка не включена; файлы и заказы не удаляются автоматически. Будущая очистка файлов/кабинетов должна быть отдельной задачей с dry-run/report.
...
`POST /checkout/orders` добавлен как отдельный endpoint для checkout-заказов сайта. Он сохраняет заказ в те же `app.orders`, `app.order_items`, `app.customers`, `app.order_delivery` и `app.order_events`, но использует `source='checkout'` и `actor_type='site'`; desktop `POST /orders` не используется для сайта.
...
Сохранённый заказ можно удалить через корзину: desktop вызывает `DELETE /orders/:id`, а затем убирает локальную карточку. Это полный hard delete заказа и связанных DB-строк; карточка клиента/customer account удаляется, если после удаления заказа не используется другими заказами. Несохранённый черновик удаляется только из `localStorage`.

Read-only загрузка заказов выполняется через `GET /orders` и `GET /orders/:id`. Desktop-лента показывает `Заказы` из общей базы отдельно от незавершённых локальных `Черновиков`.

Добавлен менеджерский workflow API без изменения DB schema: `PATCH /orders/:id/status` меняет статус заказа в существующем наборе `new`, `confirmed`, `in_work`, `ready`, `completed`, `canceled`, а `POST /orders/:id/comments` пишет комментарий менеджера в `order_events` как `comment_added`. Ручной блок `Работа менеджера` убран из desktop UI, API остаётся техническим заделом. Текущий рабочий экран менеджера сфокусирован на заявке, контактах, позициях, сумме и файлах; статусы позже должны частично автоматизироваться по реальным действиям.
...
Desktop получил локальный помощник `Подготовить оплату`: менеджер может собрать текст для клиента по сумме заказа и выбранному варианту оплаты (`счёт`, `QR-код`, `ссылка на оплату`, `оплата при получении`). Помощник ничего автоматически не отправляет, не подключает онлайн-оплату, не пишет данные в DB и не использует SMS; подготовленный текст менеджер отправляет вручную через MAX, резервно — через email.
...
Заказ с Ozon-оплатой можно удалить через `DELETE /orders/:id`: локальные payment rows удаляются вместе с заказом, active Ozon payment best-effort отменяется через provider API при наличии конфигурации, а проблемы внешнего cancel/cleanup возвращаются как cleanup warnings и не блокируют локальный hard delete. Возврат и отдельный provider workflow остаются будущим этапом.

При изменении состава, количества, цены позиций или итоговой суммы заказа с активной неоплаченной Ozon-оплатой (`created`, `pending`) API автоматически отменяет старую Ozon-оплату и только после этого сохраняет заказ. Новая Ozon Pay ссылка автоматически не создаётся. Оплаченные/финансово значимые оплаты (`authorized`, `paid`, `partial_refund`, `refunded`, `disputed`, `unknown`) блокируют изменение заказа; доплата и возврат остаются будущими отдельными workflow.
...
* MAX is the primary communication channel. Email is the fallback channel, and phone is used for calls when needed. SMS notifications, SMS provider/API, paid SMS, and sender name are out of the current plan unless a separate new decision is made.
