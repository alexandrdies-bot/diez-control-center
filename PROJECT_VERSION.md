...
Закупочные цены должны быть защищены от случайного изменения менеджером.
...
Правильная схема: `diez-site -> API -> diez-data-core -> diez-control-center`.

Главный экран будущей ПК-программы — "Заказы".

Менеджер должен обрабатывать заказы сайта из ПК-приложения.

Будущий чат сайта должен обслуживаться из приложения.

Сообщения из чата должны идти через API и общую базу.
...
Создана модель будущей навигации:
...
Зафиксировано, что текущий экран "Материалы" временно находится первым только для MVP-проверки.

В будущем он должен быть перенесён в `Настройки -> Материалы и закупочные цены`.

Главный рабочий экран должен быть "Заказы".

Роли и видимость разделов зафиксированы как будущая архитектурная модель без реализации в коде.
