...
После создания заказа в базе локальное редактирование блокируется до реализации PATCH-синхронизации. Нельзя локально менять позиции, заказчика или доставку созданного заказа, чтобы не расходиться с PostgreSQL.
