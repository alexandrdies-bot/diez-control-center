...
`POST /orders` добавлен как первый write endpoint для переноса локального desktop draft-order в общую PostgreSQL-базу через API. Desktop не пишет в PostgreSQL напрямую.
