...
Добавлен customer auth v1 и основа личных кабинетов заказчиков: регистрация по телефону и 4-значному коду, хранение кода только как hash, отдельные customer sessions и отдельный контур от manager/admin auth. В ПК-программе правильное место управления заказчиками — `Настройки -> Клиенты -> Личные кабинеты заказчиков`; `Пользователи и доступ` остаётся только для менеджеров/админов программы.
