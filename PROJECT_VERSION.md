...
Публичный личный кабинет заказчика на сайте временно заморожен. Текущий клиентский сценарий упрощается: заявки без обязательной регистрации, связь через телефон, email, MAX и чат сайта, а оплата/доставка заполняются только там, где реально есть checkout/заказ.
