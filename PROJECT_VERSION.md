...
Read-only загрузка заказов подготовлена через `GET /orders` и `GET /orders/:id`. Desktop client уже имеет методы чтения, но UI-лента остаётся MVP/localStorage до отдельного шага интеграции.
