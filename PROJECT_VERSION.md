...
Удаление заказа в ПК-программе является полным удалением заказа из БД. Клиентская сущность не удаляется автоматически. Файлы заказа удаляются после удаления DB-записей, ошибки file cleanup логируются отдельно.
...
Сохранённый заказ можно удалить через корзину: desktop вызывает `DELETE /orders/:id`, а затем убирает локальную карточку. Это полный hard delete заказа и связанных DB-строк; клиентская карточка остаётся. Несохранённый черновик удаляется только из `localStorage`.
