...
Заказ с активной неоплаченной Ozon-оплатой (`created`, `pending`, `authorized`) нельзя менять по составу или сумме. Менеджер должен сначала отменить активную оплату через backend, затем изменить заказ и создать новую Ozon Pay ссылку. Оплаченные/финансово значимые оплаты (`paid`, `partial_refund`, `refunded`, `disputed`, `unknown`) блокируют изменение заказа; доплата и возврат остаются будущими отдельными workflow.
