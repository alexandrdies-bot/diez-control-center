...
* Зафиксирована Desktop UI convention для toast/уведомлений: будущие уведомления появляются в top-right с безопасным отступом от верхних кнопок, короткие success-сообщения auto-hide через 2–3 секунды, warning/error могут жить дольше или требовать ручного закрытия, стиль остаётся в тёмной теме приложения.
* Зафиксирован Desktop UI standard: typography/icon/radius tokens, no-frame icons by default, opacity/color hover, circular bottom-right icon badges, circular close buttons and top-right success toast convention.
* Зафиксированы правила order item files/comments: клиентские файлы и комментарии показываются внутри edit screen позиции, строки позиций используют только paperclip/message индикаторы, `managerComment` сохраняется через popup/modal в item params и не влияет на финансы, CDEK/Ozon или `needsReview`; файлы менеджера к конкретной позиции требуют будущий item-level attachment mapping.
* Исправлен DTF edit flow: существующая DTF-позиция использует сохранённый `unitPriceMinor`, открытие редактора или сохранение без изменений не пересчитывает цену через default прайс, а изменение количества сохраняет прежнюю цену за единицу при том же формате.
* Исправлен Desktop attachment preview flow: карточки файлов остаются 3:5, thumbnail и modal используют authenticated blob/preview flow, image detection учитывает MIME type и common image extensions, download/open поведение не менялось.
* CDEK UI закреплён как preliminary delivery flow: нижняя общая кнопка `Сохранить` не используется в CDEK panel, сохранение идёт одной кнопкой в блоке доставки, а будущая кнопка `Создать отправление в СДЭК` должна появиться только после оплаты заказа, сохранённой доставки без `needsReview` и до создания реального отправления.
...
* Удаление заказа приведено к правилу полного hard-delete: менеджер удаляет заказ из рабочей базы вместе с позициями, доставкой, оплатами, вложениями и историей.
* Удаление заказа теперь также удаляет связанную карточку клиента/customer account, если после удаления заказа они больше не используются другими заказами.
...
* Заменено старое правило удаления заказа с Ozon-оплатой: `DELETE /orders/:id` больше не блокирует локальный hard-delete из-за payment rows; возможные проблемы внешнего cancel/cleanup логируются и возвращаются как предупреждения.
