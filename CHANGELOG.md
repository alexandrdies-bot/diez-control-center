...
* Добавлена защита редактирования заказа с Ozon-оплатой: активные неоплаченные оплаты нужно сначала отменить через `POST /orders/:id/payments/:paymentId/cancel`, а оплаченные/финансово значимые оплаты блокируют изменение заказа до будущего workflow доплат/возвратов.
