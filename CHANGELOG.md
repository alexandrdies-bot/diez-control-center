...
* Добавлена MVP production protection для API: CORS allowlist через env, write key для `POST /orders` и `DELETE /orders/:id`, admin key для `PATCH /materials/:id/pricing-inputs`; dev режим остаётся открытым без ключей.
