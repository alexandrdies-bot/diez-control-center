...
Заказ с активной неоплаченной Ozon-оплатой (`created`, `pending`, `authorized`) нельзя менять по составу или сумме: сначала менеджер отменяет оплату, затем правит заказ и создаёт новую ссылку. Оплаченные и финансово значимые оплаты (`paid`, `partial_refund`, `refunded`, `disputed`, `unknown`) блокируют изменение заказа; доплата и возврат будут отдельным будущим сценарием. Если все Ozon-оплаты только `failed`, `canceled` или `expired`, заказ можно менять.
