...
Менеджерский workflow заказа добавлен поверх существующей схемы БД без новых миграций: `PATCH /orders/:id/status` меняет статус заказа и пишет событие `order_status_changed`, а `POST /orders/:id/comments` добавляет комментарий менеджера в историю `order_events` с типом `comment_added`. Desktop показывает блок `Работа менеджера` в деталях заказа: текущий статус, смену статуса, комментарий к смене статуса, отдельный комментарий менеджера и простую историю событий.
