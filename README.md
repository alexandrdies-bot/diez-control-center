...
Заказ с любой Ozon-оплатой нельзя удалять через `DELETE /orders/:id`: связь заказа и финансового следа должна сохраняться даже для pending/paid/failed/expired оплат. Отмена и возврат Ozon Pay будут отдельным workflow; текущая защита не вызывает Ozon cancel/refund API.
