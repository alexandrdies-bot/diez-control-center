...
## Temporary development SSH deploy access

Для временного deploy-доступа во время разработки используется отдельный SSH key. Ключ создан локально только для development/deploy задач; публичная часть добавляется на сервер в `~/.ssh/authorized_keys`, а приватная часть хранится только локально и не коммитится.

Root-пароль нельзя писать в чат, сохранять в проекте, документации, `outputs` или других файлах репозитория. После окончания этапа разработки временный ключ нужно удалить из серверного `~/.ssh/authorized_keys`.

Deploy через этот доступ разрешён только для ограниченных операций: `git pull`, сборка нужного проекта, перезапуск конкретного `systemd` service и health-check. Через этот доступ нельзя менять `.env`, nginx, Basic Auth, PostgreSQL, seed-данные или migrations без отдельного подтверждённого задания.

## Desktop UI conventions

Toast/уведомления в Desktop UI должны появляться в верхнем правом углу окна, с безопасным отступом сверху и справа, чтобы не перекрывать `+ Новый заказ`, cloud/status и настройки. Обычные success-сообщения должны быть короткими и скрываться автоматически через 2–3 секунды; warning/error могут жить дольше или требовать ручного закрытия. Стиль уведомлений должен соответствовать тёмной теме Desktop UI.

Desktop UI standard: typography uses fixed levels for page, modal, section, card, body, caption, label and button text; icons use `sm/md/lg` sizes. Icons are rendered without frames/background wrappers unless explicitly requested, hover uses opacity/color, dot/count badges are circular and placed at the icon bottom-right corner, close buttons are always circular, and modal/card/button/input radii come from shared CSS tokens.

Order item files/comments UI: customer comments and customer files are shown inside the corresponding item edit screen. Order item rows show only compact paperclip/message indicators, never file previews or full comment text. Manager comments are stored in item params as `managerComment`, open through a popup/modal, and must not change item price, order total, delivery `needsReview`, Ozon, or CDEK state. Manager file upload to a specific item is intentionally not enabled until an item-level attachment mapping exists (`order_item_id` or metadata `itemLocalId`/`orderItemId`).
...
```text
D:\_ProjectHome\Eco_System_Diez\diez-data-core
```

Сайт находится здесь:

```text
D:\_ProjectHome\Eco_System_Diez\diez-site
```
...
## Customer auth и личные кабинеты заказчиков

Customer auth v1 отделён от manager/admin auth. Заказчик регистрируется по телефону и 4-значному коду, а код хранится только как hash. Manager/admin auth остаётся отдельным входом для пользователей ПК-программы.

Публичный личный кабинет заказчика на сайте временно заморожен. Текущий клиентский сценарий должен оставаться проще: заявки без обязательной регистрации, основной канал ответа клиенту — MAX, резервный канал — email, телефон используется для звонка при необходимости, а оплата/доставка заполняются только там, где реально есть checkout/заказ. SMS-уведомления исключены из текущего плана: SMS-провайдер, SMS API, платные SMS и имя отправителя сейчас не подключаются и не планируются без отдельного нового решения. В DTF и будущих заявках клиент не выбирает SMS.

Уже созданные `app.customer_accounts`, `app.customer_sessions`, customer auth API, customer profile API, customer orders API и retention-поля не удалять. Это задел на будущее, но его нельзя считать обязательным текущим публичным сценарием.

Личный кабинет заказчика — временное рабочее пространство для выполнения, обсуждения и доработки заказов. Это не вечный файловый архив. Ориентир хранения — 3 месяца после последней активности; автоочистка пока не включена.

В навигации ПК-программы раздел `Пользователи и доступ` предназначен только для пользователей программы: менеджеров и администраторов. Экран управления личными кабинетами заказчиков убран из ПК-программы на время заморозки публичного кабинета. Заказчиков нельзя смешивать с пользователями программы, а уже созданная DB/API-модель остаётся заделом на будущее.

Retention-поля подготовлены для будущей политики хранения. `retention_locked` означает запрет автоочистки личного кабинета и нужен для постоянных клиентов, рекламных агентств и важных заказчиков. Пока очистка файлов и кабинетов не запускается.

Тяжёлые файлы заказов не должны храниться вечно, но пока заказ активен, файлы нельзя автоматически удалять. Автоочистка не включена; файлы и заказы не удаляются автоматически. Будущая очистка должна быть отдельной задачей с dry-run/report.

## Ozon Pay Checkout payments

Добавлен backend API-слой Ozon Pay Checkout для будущей оплаты заказов: создание платежа по заказу, получение списка оплат, ручная синхронизация статуса и публичный webhook для уведомлений Ozon. UI в ПК-программе пока не добавлен, автоматическая отправка клиенту, SMS, личный кабинет и автоматическая смена статусов заказа не включались.

Ozon secrets должны храниться только на сервере в env. Desktop и сайт не должны хранить `accessKey`, `secretKey`, notification secret или другие платёжные секреты. Если Ozon env не настроен, manager endpoints отвечают `OZON_ACQUIRING_NOT_CONFIGURED`, а сервер при старте не падает.

Webhook route `POST /payments/ozon/webhook` нужно будет открыть через nginx отдельным шагом перед production-проверкой. Ozon API вручную не вызывается без настроенного env и отдельной проверки.

Менеджерское удаление заказа через `DELETE /orders/:id` является полным hard delete рабочей записи заказа: локальные строки заказа, включая Ozon payment metadata, удаляются вместе с заказом. Связанная карточка клиента/customer account тоже удаляется, если после удаления заказа не используется другими заказами. Если есть активная Ozon-оплата, API может попытаться вызвать `cancelOrder`, но ошибка внешней отмены не блокирует удаление локального DB-заказа и возвращается как cleanup warning. Внешняя отмена/возврат у provider может быть отдельным будущим workflow.

Если менеджер меняет состав, количество, цену позиций или итоговую сумму заказа с активной неоплаченной Ozon-оплатой (`created`, `pending`), API сначала автоматически отменяет старую Ozon-оплату, затем сохраняет изменения заказа. Новая Ozon-оплата автоматически не создаётся: после проверки менеджер вручную создаёт новую ссылку. Оплаченные и финансово значимые оплаты (`authorized`, `paid`, `partial_refund`, `refunded`, `disputed`, `unknown`) блокируют изменение заказа; доплата и возврат будут отдельным будущим сценарием. Если все Ozon-оплаты только `failed`, `canceled` или `expired`, заказ можно менять.
...
Desktop API client имеет wrappers для этих backend CDEK endpoints: status, cities, delivery points, calculation и save selected delivery. Desktop не ходит напрямую в СДЭК и не хранит CDEK credentials или OAuth token.

Desktop delivery panel теперь имеет компактный CDEK calculation/save UI-блок: статус backend CDEK, поиск города, ПВЗ, tariffCode, вес/габариты, кнопка расчёта, результат и отдельная кнопка `Сохранить доставку`. Блок использует только backend API wrappers, не ходит напрямую в CDEK, не хранит CDEK credentials/OAuth token, не создаёт shipment, не создаёт новую Ozon payment и не меняет статус заказа автоматически.
...
Для CDEK calculator поле provider `type` не используется как режим доставки склад/ПВЗ: по OpenAPI это order type, поэтому backend не отправляет туда значения delivery mode `1/2/3/4`. Выбранные пункты задаются через `shipment_point` и `delivery_point`, а `currency` остаётся numeric `1` для RUB.

Backend API имеет endpoint `PATCH /orders/:id/delivery/cdek` для сохранения выбранной СДЭК-доставки как delivery summary заказа. Он пишет только существующие `app.order_delivery` и `app.orders` delivery/total fields, не создаёт отправление, не использует `app.order_shipments` / `app.order_shipment_packages`, не вызывает CDEK API и не получает OAuth token. Если изменение доставки влияет на сумму заказа и уже была active Ozon payment, backend отменяет старую оплату; новую Ozon Pay ссылку менеджер создаёт вручную после проверки доставки. Final/financial payments блокируют изменение. Response возвращает `payment.financialChanged`, `payment.activePaymentsCanceled`, `payment.canceledPaymentIds` и `payment.deliveryNeedsReview`.

Текущий CDEK flow в Desktop — только предварительная доставка для расчёта суммы заказа: менеджер выбирает маршрут, ПВЗ, груз и тариф, затем сохраняет доставку в заказ, чтобы пересчитать итог `товары + доставка` и подготовить оплату. СДЭК-доставка отображается отдельной визуальной строкой под товарными позициями и отдельной строкой в итогах, но не превращается в обычную товарную позицию `order_items`; товарные позиции, например DTF-печать, остаются отдельным списком. Если менеджер изменил товарные позиции при уже сохранённой доставке и была active Ozon payment, backend отменяет старую оплату, но откладывает создание новой ссылки до перепроверки/сохранения доставки, чтобы не выставить оплату с устаревшей доставкой. `Сохранить доставку` не создаёт реальное отправление СДЭК, трек-номер, накладную, документы или этикетки. Реальное `Создать отправление в СДЭК` будет отдельным будущим workflow, когда заказ готов к отправке; будущая кнопка должна быть доступна только после оплаты заказа, при сохранённой СДЭК-доставке без `needsReview` и до создания реального отправления.
...
CDEK delivery UI в Desktop работает в summary-first режиме: сохранённая доставка сначала показывается компактной карточкой с маршрутом, тарифом, стоимостью, сроком, грузом и отметкой `Отправление СДЭК ещё не создано`, а полная форма открывается только для изменения или пересчёта. Статус интеграции отображается как маленькая техническая строка, списки городов/ПВЗ/тарифов ограничены по высоте и сворачиваются после выбора, а нижняя общая кнопка `Сохранить` не используется для сохранения СДЭК-изменений.

CDEK edit/new flow не должен открываться технической простынёй или конструктором из скрытых городов. Внутри CDEK panel используется обычная форма доставки: `Маршрут`, `Груз`, `Тарифы`, `Расчёт и сохранение`. В маршруте город отправителя и город получателя видны как поля ввода сразу; после ввода валидного запроса Desktop запускает autocomplete с debounce и показывает dropdown под активным полем, а ПВЗ/пункты открываются только для выбранного города как select-like поля.

При переходе из saved summary в CDEK edit mode Desktop повторно гидратирует форму из сохранённого `delivery.providerPayload`: sender/recipient city and point, тариф, цена, срок и груз остаются заполненными, а active picker по умолчанию закрыт. CDEK-блок работает как форма оформления доставки: города ищутся автоматически с debounce и dropdown под полем, результаты закрываются после выбора, ПВЗ/пункты показываются только в активном select-like выборе, тарифы выводятся карточками после заполнения маршрута и груза, а технические коды остаются вторичными. Сохранённая доставка остаётся помеченной как сохранённая, пока менеджер не меняет данные.

CDEK city/point/PVZ/tariff dropdowns открываются overlay-слоем и не растягивают форму. Пункты отправки и ПВЗ выбираются через select-like поле с локальным поиском по адресу и строкой количества показанных пунктов; тарифы показываются вертикальным списком в стиле оформления доставки, с названием слева, ценой справа, сроком ниже и вторичным `#code`. Desktop не скрывает возвращённые backend пригодные тарифы из-за локального лимита карточек; backend contracts, CDEK provider calls, DB, Ozon и shipment lifecycle не меняются.

Блок `Тарифы` в CDEK delivery UI работает как отдельная рабочая секция: менеджер видит режимы `Откуда` и `Куда доставить`, где текущий MVP-сценарий `из ПВЗ -> в ПВЗ` активен, а неподдержанные courier/postamat варианты disabled. Когда маршрут или груз ещё не заполнены, секция просит заполнить данные; когда они готовы, Desktop автоматически подбирает тарифы, показывает loading `Подбираем тарифы...`, а затем выводит вертикальный список, где выбранная строка подсвечивается.

После заполнения маршрута и груза Desktop автоматически подбирает CDEK tariffs через существующий backend wrapper и запоминает fingerprint route/cargo, чтобы не повторять запрос без изменения данных. Основной tariff list показывает релевантные PVZ→PVZ/`склад-склад` варианты, сортируя `Посылка`, `Экспресс`, `Магистральный экспресс`, `Супер-экспресс` выше прочих; остальные provider варианты не удаляются, а прячутся в `Другие варианты`. Если выбранный tariff list row уже содержит price/period, Desktop формирует calculation state из этих нормализованных данных, а save delivery по-прежнему идёт только через backend save endpoint.

Для диагностики отсутствующего `Посылка склад-склад` backend `POST /cdek/tariffs` пишет безопасный structured log только по нормализованным тарифам: total/valid/errored counts, tariff code/name/mode/error flags, presence `#136`/`Посылка` и склад-склад counts. Лог не содержит Authorization, bearer/OAuth token, CDEK credentials, raw env, raw provider request/response или персональные данные.
...
```text
PostgreSQL из D:\_ProjectHome\Eco_System_Diez\diez-data-core
```
...
В desktop добавлен локальный помощник `Подготовить оплату`: он формирует текст для клиента по номеру и сумме заказа, поддерживает варианты `счёт`, `QR-код`, `ссылка на оплату`, `оплата при получении`, но ничего автоматически не отправляет и не сохраняет данные в БД. SMS не используется; основной рабочий канал для отправки подготовленного текста остаётся MAX, резервный канал — email.

Менеджерский workflow API добавлен поверх существующей схемы БД без новых миграций: `PATCH /orders/:id/status` меняет статус заказа и пишет событие `order_status_changed`, а `POST /orders/:id/comments` добавляет комментарий менеджера в историю `order_events` с типом `comment_added`. Ручной блок `Работа менеджера` убран из desktop UI, чтобы не усложнять работу менеджеров: текущий рабочий экран заказа оставляет заявку, контакты, позиции, сумму, файлы и кнопку добавления позиции. Статусы позже должны частично автоматизироваться по реальным действиям.

`DELETE /orders/:id` удаляет уже созданный заказ через hard delete из `app.orders`. Связанные строки `order_items`, `order_delivery`, `order_events`, `order_attachments`, `order_payments`, `order_shipments` и `order_shipment_packages` удаляются каскадом. Связанные `customers` и `customer_accounts` также удаляются, если после удаления заказа они больше не используются другими оставшимися заказами; если customer используется другим заказом или у него остаются accounts, карточка сохраняется и API возвращает cleanup warning. `customer_sessions` удаляются через каскад customer account, если это поддерживает текущая схема. Физические файлы вложений удаляются после успешного DB commit через best-effort cleanup; ошибки cleanup логируются и могут вернуться в `cleanupWarnings`, но не откатывают удалённый DB-заказ. Позже можно добавить отдельный UI для поиска и повторного использования карточек клиентов.
...
```text
D:\_ProjectHome\Eco_System_Diez\diez-control-center
  apps
    desktop
    api
  packages
...
```text
D:\_ProjectHome\Eco_System_Diez\diez-shared-core\packages\calculation-core
```
...
The feed trash action has two modes: unsaved local drafts are removed from `localStorage`; saved orders call `DELETE /orders/:id` first and then remove the local draft card after successful API deletion. Saved-order deletion is a full hard delete of the order and its DB child rows; the customer/account record is also removed when no remaining order uses it, while reused customer records stay in place with a cleanup warning. External provider cleanup/cancel can be handled separately from the local DB delete.
...
MAX — это основной канал связи. Email остаётся резервным каналом, телефон используется для звонка при необходимости. ИИ — мозг для разбора заявки и подготовки ответа. База/API — место хранения сообщений, файлов, клиентов и черновиков заказов. ПК-программа — рабочее место менеджера. SMS-провайдер, SMS API, платные SMS и имя отправителя сейчас не подключаются и не планируются без отдельного нового решения.
