...
CDEK delivery UI в Desktop работает в summary-first режиме: сохранённая доставка сначала показывается компактной карточкой с маршрутом, тарифом, стоимостью, сроком и грузом, а полная форма открывается только для изменения или пересчёта. Статус интеграции отображается как маленькая техническая строка, списки городов/ПВЗ/тарифов ограничены по высоте и сворачиваются после выбора, а нижняя общая кнопка `Сохранить` не используется для сохранения СДЭК-изменений.
